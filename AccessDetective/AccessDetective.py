...
def deneme_panel(panel):
    birles = url + panel
    r = requests.get(birles)

    if r.status_code == 200:
        print("[+] Panel Bulundu ! --> " + birles)
        with open("AdminPanel.txt", "a") as dosya:
            dosya.write("[+] Panel Bulundu ! --> " + birles + "\n")
            dosya.write("=" * 50 + "\n")  # Görsel ayırıcı
            global sayac
            sayac += 1
    elif r.status_code == 403:
        print("[!] Erişim Engellendi ! --> " + birles)
    elif r.status_code == 404:
        print("[-] Olumsuz ! --> " + birles)
...
time.sleep(1)
print("\nListe denenmek için hazırlanıyor...")
time.sleep(2)
print("\nEmir Byk | Klasik Bilişim </>")
time.sleep(3)

liste = ['wp-admin/','admin/','administrator/','login.php','administration/','admin1/','admin2/','admin3/','admin4/','admin5/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
threads = []
for panel in liste:
    thread = threading.Thread(target=deneme_panel, args=(panel,))
...
if sayac == 0:
    print("[-] Hedef Sitenin Paneli Bulunamadı !")
else:
    with open("AdminPanel.txt", "a") as dosya:
        dosya.write("[+] Toplam Panel Sayısı: " + str(sayac) + "\n")
    print("[+] Sonuçlar AdminPanel.txt dosyasına kaydedildi!")

print("Toplam süre: {:.2f} saniye".format(elapsed_time))